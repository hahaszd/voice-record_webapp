...
_HALLUCINATION_PHRASES = [
    ...
    # 通用重复幻觉检测（不依赖硬编码某句原文）：整段输出只是同一个 1-6 字短词/短语，
    # 被空格或逗号类分隔符隔开、重复 3 次及以上 —— 这是 Whisper 对着静音/噪音吐出的
    # 典型幻觉形状（例如 "栏目 栏目 栏目"，2026-07-23 用户报告，静音录音触发）。
    # 加 ^…$ 锚定整段匹配：只在"重复项之间有分隔符、且整段输出别无他物"时才命中——
    # 真实中文口语的紧凑重复（如"对对对"，字词间无分隔符）或夹在正常句子中间的重复
    # 一律不受影响，避免误删真实语音（参见上面 v114 的教训）。
    r'^(\S{1,6})(?:[\s,，、]+\1){2,}[\s,，、。!！?？]*$',
]
