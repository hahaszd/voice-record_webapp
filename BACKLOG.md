...
### 🔵 whisper-1 输出繁體 → 加繁转简后处理（2026-07-28）

**为什么重要**：实测确认 `whisper-1` 对普通话**输出繁體字**（"我想錄幾句話…"），而
**`language` 参数改不了这个**（`auto` / `zh` 两次实测结果相同），且**代码里没有任何繁转简处理**
（已 grep 全仓库确认）。所以你现在每一条中文转录拿到的都是繁體 —— 你自己全程用简体，
这是一个持续的、此前没被识别出来的体验损耗。

**这跟换模型无关** —— 模型选型已结案（见上），这个缺点用后处理解决。

**待办**：在转录结果返回前加一步繁→简转换。
- 候选方案：`opencc-python-reimplemented` 或 `zhconv`（后者纯 Python、无二进制依赖、体积小，
  对 Railway 构建更友好）
- 位置：`api_fallback.py` 的文本后处理链，与 `_scrub_hallucination_phrases` 同层
- ⚠️ **要不要做成可选**：如果将来有繁體用户，硬转简会伤到他们。当前是个人自用工具
  （见 CLAUDE.md 的 tool philosophy），**建议先无条件转，别过度设计**，真有需求再加开关
- ⚠️ 注意别碰系统音路径的既有行为，先只改麦克风路径并实测
