...
| M2 | 息屏/切后台后 mic 恢复（AUDIO-HEALTH） | 恢复录音、track 可用性检测 | L4 | P1 | 🟡 **提醒已做，恢复未做**（v122）：录音中 `track.onmute` → 红色提示条 + 系统通知；**录音中途自动换流仍未实现**，仍需用户手动停止重开。自动换流是本条剩余部分 |
...
| ID | 验证内容 | 期望 | Layer | 优先级 | 现状 |
|----|----------|------|-------|--------|------|
| N1 | 静音/杂音幻觉过滤 | no_speech_prob/compression_ratio/avg_logprob 阈值过滤 | L1 | P1 | ✅ `tests/backend/test_hallucination_filter.py`（18 条）。按 review 结论**只断言行为与不变式、不锁阈值数值**：极端取值验明确该丢/该留；位置感知用**网格扫描**验集合包含关系；边界取值由探测得出而非硬编码。**经 5 项变异测试验证确能抓到回归**（详见下方说明）。含 v129 文本层通用重复正则与误伤防护。**不含 N2** —— 本条只验到"全幻觉→抛异常触发降级"这一层，N2 要的"最终给用户看到'未识别到文字'"是端到端行为，仍待建 |
...
- **测试资产**：前端 8 个 eval spec（L0/L2/L3，含 fake-mic 录音生命周期、静态资源 cache-bust）+ 后端 pytest 31 个（降级引擎 + 限流 + 幻觉过滤）。
...
### 2026-07-28 N1 补测完成（含变异测试验证）

`tests/backend/test_hallucination_filter.py`，18 条，纯函数、零 mock、零网络。

**写法约定**：阈值是明确要按真实 Railway 数据调的，所以**不锁具体数值** —— 锁死只会让人
改测试迁就代码。明确该丢/该留的用例一律用**极端取值**（任何合理阈值下结论都不变）；
位置感知用**网格扫描**比较集合包含关系；`start<3.0` 的边界取值由**探测**得出，不硬编码。

**⚠️ 变异测试暴露的坑（值得记住的方法论）**：最初写的"开头判定严于非开头"不变式
**抓不到"阈值被放松"** —— 因为非开头是**合取**、开头是**析取**，即使把开头三个阈值改成
与非开头完全相同，开头依旧因结构差异而"更严"，断言恒成立、测试假绿。
只有 `compression_ratio`（两边都是单指标析取，2.0 vs 2.4）在**其余指标压到无害后单独隔离**
才能照出阈值放松。为此专门补了一条用例。

**5 项变异测试全部变红**（即测试确有保护力）：① 开头阈值放松成与非开头相同；
①b 只放松 compression 一个阈值；② 非开头改回 v114 前的单指标判定；③ 去掉 `start<3.0` 条件；
④ 删掉 v129 通用重复正则；⑤ 全段幻觉时返回空串而非抛异常。

**顺带澄清一处非 bug**：空 segments 列表会抛异常（`0 == 0` 命中"全部被丢"分支）。看似
可疑，实则生产不可达（`api_fallback.py:886` 的 `if not text: raise` 先兜住了），且即便到达，
抛异常→降级下一个 API 也是更安全的结果。**故意不改生产代码去迁就一个更好看的返回值**，
改为在测试里记录真实行为与原因。

### 2026-07-28 更新（v122 补录 handover）
- **v122/v129 两笔改动上线时未补测试**：`api_fallback.py` 的幻觉过滤（开头段落阈值、通用重复正则、
  AI Builder 补齐段落过滤）+ `static/script.js` 的录音中 mic 失效提醒，**全部只有手工验证**。
  既有 13 个后端 pytest 覆盖的是降级引擎与限流，**碰不到这些改动**。
- **⭐ 当前最高性价比缺口 = N1**：过滤逻辑现在是纯函数，写 L1 单测几乎零成本，却能兜住一处
  自认"未经真实数据验证"的阈值。**建议作为下一个动作**。
- **M2 降级为"仅剩自动换流"**：提醒部分已上线，剩下的是录音中途安全换 track（改动较大，未排期）。
- 结论：**P0 仍为 0**，但 N1 建议从 P1 提为事实上的下一优先项。
