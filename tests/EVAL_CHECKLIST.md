...
| ID | 验证内容 | 期望 | Layer | 优先级 | 现状 |
|----|----------|------|-------|--------|------|
| N1 | 静音/杂音幻觉过滤 | no_speech_prob/compression_ratio/avg_logprob 阈值过滤 | L1 | P1 | ⬜ **优先级实际上升**（v122）：过滤逻辑已抽成纯函数 `_filter_hallucinated_segments(segments, log_tag)`，**零 mock、零网络即可测**，成本已经很低。且 v122 新增的**开头段落阈值是未经真实数据验证的推断**、v129 的通用重复正则也只有手工验证——目前这两处改动**没有任何自动化测试兜底**。建议按 review 结论只做**存在性/形状断言**（幻觉前缀被剥离、全幻觉抛异常走 fallback、"对对对"不误伤），**不锁具体阈值数值** |
