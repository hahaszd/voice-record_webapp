...
cache-bust 129），且**都没有唯一权威来源**。实证：`0cb7c01「v129」`用的是 cache-bust 号却比
`aa747c5「v122」`提交更早；cache-bust 自身 2026-02-09 从 106 倒退回 103。
**做法**：`APP_VERSION`（`server2.py`）设为功能号唯一来源；cache-bust 改为服务端注入内容哈希
（三个 handler 本就是"读成字符串再返回"，无需模板引擎）；HTML 用 `?v=auto` 占位。
**明确不做**（owner 认可）：不重编约 205 处历史 `vNNN`（考古坐标，重编等于烧掉）；
不换 semver/日期版本号（vNNN 本身没毛病，问题是撞了 cache-bust，干掉撞的那个即可）。
**连带废除 CLAUDE.md 的「CACHE-BUST 铁律」** —— 该人工规则至少漏过两次，见下条。

**`发现` 真 bug：about/faq 的老访客一直被喂 2 月的样式表**
`about.html`/`faq.html` 引用 `style.css?v=105`（2026-02 定），而 `style.css` 7 月改过。
浏览器按完整 URL 缓存 → **回访这两页的用户拿的是五个月前那份样式表**。
（`audio-storage.js?v=51` 是虚惊：该文件 2 月后未再改动。）
已由 v123 的自动哈希一并修复，并加 `tests/smoke/asset-versioning.spec.ts` 盯住 ——
该机制**静默失效**时页面照常渲染，只是回访用户被永久钉死在 `?v=auto` 这个缓存键上。
